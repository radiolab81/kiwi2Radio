...
HELP_MESSAGE_LIST = ["KiwiSDR 2 Radio %s HELP" % VERSION,
        "",
...
class kiwi_list():
    ...
    def choose_kiwi_dialog(self):
        ...
        l = tkinter.Label(self.root, text = "Choose a KiwiSDR")
        l.config(font =("Mono", 12))
        l.pack()
    ...
    def refresh_list(self):
        ...
        for idx, kiwi_record in enumerate(self.kiwi_list):
            ...
            self.listbox.insert(END,kiwi_string)
    ...
    def connect_new_kiwi(self, save_flag=False):
        ...
        print("new_host" , self.kiwi_host)
        print("new_port" , self.kiwi_port)
        print("new_passwd" , self.kiwi_password)
        
        self.connect_new_flag = True
        if self.connect_new_flag:
        	self.root.destroy()
        #if save_flag:
        #	self.save_to_disk()
...
class display_stuff():
    ...
    def splash_screen(self, sdrdisplay):
        ...
        logo_pic = pygame.image.load('logo.png')
        logo_pic = pygame.transform.scale(logo_pic, (self.DISPLAY_WIDTH, self.DISPLAY_HEIGHT))
        sdrdisplay.blit(logo_pic, (0, 0))
        
        block = font.render(" - KiwiSDR 2 Radio - ", True, RED)
        rect = block.get_rect()
        rect = block.get_rect(center=(self.DISPLAY_WIDTH/2, self.DISPLAY_HEIGHT/2-90))
        sdrdisplay.blit(block, rect)
        block = font.render("...CONNECTING...", True, RED)
        rect = block.get_rect(center=(self.DISPLAY_WIDTH/2, self.DISPLAY_HEIGHT/2+90))
        sdrdisplay.blit(block, rect)
