...
# config is done via OptionParser now
# this port address is for the serial tx/rx pins on the GPIO header of your I/O Interface
#PORT = '/dev/ttyUSB0'
# be sure to set this to the same rate used on the I/O Interface
#RATE = 115200
...
parser = OptionParser()
parser.add_option("-i", "--serialdev", type=str,
                  help="path to serial io interface", dest="PORT", default="/dev/ttyUSB0")
parser.add_option("-t", "--serialbitrate", type=int,
                  help="bitrate of serial io interface", dest="RATE", default=115200)
...
pygame.display.set_caption("KiwiSDR 2 Radio")
...
while not wf_quit:
    ...
    #manual_snd_freq = int(8475)
    # waiting for commands from I/O interface
    if serialport.in_waiting:
        try:
            reading = serialport.readline().decode('utf-8')
    	    ...
    	    if "FRX=" in reading:
    		    ...
    		    reading = reading.replace("FRX=",'')
    		    reading = reading.replace("=",'');
    		    reading = reading.strip()
    		    try:
    		        float(reading)
    		    except ValueError:
    		        print("FRX= not a float")
    		    else:
    		        manual_snd_freq = float(reading)
    		        #manual_snd_freq = float(reading[4:len(reading)])
    		        #if reading[4:len(reading)].isnumeric():
